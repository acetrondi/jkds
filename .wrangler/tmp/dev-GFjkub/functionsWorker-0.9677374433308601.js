...
// .wrangler/tmp/pages-sauTWf/functionsWorker-0.9677374433308601.mjs
...
async function generateToken2(username, password) {
  const encoder = new TextEncoder();
  const data = encoder.encode(`${username}:${password}`);
  const hashBuffer = await crypto.subtle.digest("SHA-256", data);
  return Array.from(new Uint8Array(hashBuffer)).map((b) => b.toString(16).padStart(2, "0")).join("");
...
async function isValidToken(token, env) {
  const expected = await generateToken2(env.ADMIN_USERNAME, env.ADMIN_PASSWORD);
  return token === expected;
}
__name(isValidToken, "isValidToken");
__name2(isValidToken, "isValidToken");
...
async function generateToken3(username, password) {
  const encoder = new TextEncoder();
  const data = encoder.encode(`${username}:${password}`);
  const hashBuffer = await crypto.subtle.digest("SHA-256", data);
  return Array.from(new Uint8Array(hashBuffer)).map((b) => b.toString(16).padStart(2, "0")).join("");
}
__name(generateToken3, "generateToken3");
__name2(generateToken3, "generateToken");
async function isValidToken2(token, env) {
  const expected = await generateToken3(env.ADMIN_USERNAME, env.ADMIN_PASSWORD);
  return token === expected;
}
__name(isValidToken2, "isValidToken2");
__name2(isValidToken2, "isValidToken");
async function onRequestPost3({ request, env }) {
  ...
  if (!await isValidToken2(body.token, env)) {
    return new Response("Unauthorized", { status: 401 });
  }
  const [owner, repo] = env.GITHUB_REPO.split("/");
  const branch = "main";
...
// .wrangler/tmp/bundle-IprZBp/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-IprZBp/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.9677374433308601.js.map
